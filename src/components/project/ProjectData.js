import r2 from "../../img/r2.jpg";
import r3 from "../../img/r3.jpg";
import r4 from "../../img/r4.jpg";
import r5 from "../../img/r5.jpg";
import r6 from "../../img/r6.jpg";
import travel from "../../img/travel.jpg";

const Data = [
  {
    id: 7,
    image: travel,
    title: "Travel Pack App",
    desc: "An app which allows users to make a list of all goods required for travelling, and also keep the track of goods whether they are packed or not. ",
    skills: "React, Javascript, CSS",
    demo_link: "https://travel-list-six.vercel.app/",
    github_link: "https://github.com/nirajan1985/travel-list",
  },
  {
    id: 2,
    image: r2,
    title: "Notes App",
    desc: "An app which allows users to make notes of tasks conveniently.",
    skills: "React, Typescript, Bootstrap",
    demo_link: "https://notesappmy.netlify.app/",
    github_link: "https://github.com/nirajan1985/notesapp",
  },
  {
    id: 3,
    image: r3,
    title: "Currency Converter",
    desc: " A live currency converter app for converting money from one currency to other instantly with the help of API used.",
    skills: "React, Javascript, API (Exchange rates data api)",
    demo_link: "https://mycurrency-converter.vercel.app/",
    github_link: "https://github.com/nirajan1985/currencyconverter",
  },
  {
    id: 4,
    image: r4,
    title: "Youtube Clone",
    desc: " A youtube clone where users can search and play videos.",
    skills: "React, Javascript, GoogleApi, Semantic UI",
    demo_link: "https://video-hooks-ten.vercel.app/",
    github_link: "https://github.com/nirajan1985/youtube_clone",
  },
  {
    id: 5,
    image: r5,
    title: "Task Tracker",
    desc: " An app for tracking tasks which can be used as a Reminder. Users can also add a new task, delete and modify it.",
    skills: "Vue, CSS, Javascript, Typescript",
    demo_link: "https://taskplan.netlify.app/",
    github_link: "https://github.com/nirajan1985/task-tracker",
  },
  {
    id: 6,
    image: r6,
    title: "Restaurant Menu",
    desc: "A responsive restaurant QR-code menu which allows users to browse restaurant's menu in their mobile.",
    skills: "React, Media Queries",
    demo_link: "https://1947gandhi-menu.vercel.app/",
    github_link: "https://github.com/nirajan1985/restaurant_menu",
  },
];

export default Data;
